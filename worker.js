      ...
      if (upgradeHeader === "websocket") {
        ...
        if (url.pathname.length == 3 || url.pathname.match(",")) {
          ...
          if (!kvPrx[prxKey] || kvPrx[prxKey].length === 0) {
            return new Response(`Proxy parameter ${prxKey} not found in proxy list.`, { status: 404 });
          }
...
async function handleTCPOutBound(
  ...
  try {
    ...
    log("initial connection failed, attempting retry", e);
    await retry();
  }
}
